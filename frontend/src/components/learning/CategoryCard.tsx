      ...
      <Card hoverable className={compact ? 'p-5 h-full' : 'p-6 flex flex-col gap-4'}>
        ...
        {progress && (
          <div className="space-y-3 rounded-xl bg-slate-50 p-3">
            <div className="flex items-center justify-between text-xs font-bold text-slate-600"><span>התקדמות</span><span>{progress.lessons_completed}/{progress.lessons_total} שיעורים</span></div>
            ...
            <div className="flex items-center justify-between gap-2 text-xs">
              <span className="inline-flex items-center gap-1 text-slate-500"><CheckCircle2 className="w-3.5 h-3.5 text-emerald-500" />{hasPractice ? `${progress.accuracy_percent}% דיוק` : 'טרם תרגלת'}</span>
              <span className="inline-flex items-center gap-1 font-bold text-indigo-600"><Target className="w-3.5 h-3.5" />{progress.level}</span>
            </div>
          </div>
        )}
        ...
        {compact && category.lessons.length > 0 && (
          <div className="mt-auto pt-3 border-t border-slate-100 flex items-center justify-between text-xs text-slate-500">
            <span>{progress && progress.lessons_completed > 0 ? 'המשך במסלול' : `השיעור הראשון: ${category.lessons[0].title}`}</span>
            <Link href={`/lesson/${category.lessons[Math.min(progress?.lessons_completed ?? 0, category.lessons.length - 1)].id}`} className="inline-flex items-center gap-1 text-indigo-600 font-bold hover:text-indigo-700 shrink-0" onClick={(event) => event.stopPropagation()}>
