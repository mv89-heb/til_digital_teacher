...
export default function LearnPage() {
  ...
  const progressLessons = useMemo(() => {
    ...
    return [
      ...(dashboard?.in_progress_lessons ?? []),
      ...(dashboard?.completed_lessons ?? []),
    ] as LessonProgressEntry[];
  }, [dashboardQuery.data]);

  const progressMap = useMemo(
    () => new Map(progressLessons.map((item) => [item.lesson_id, item])),
    [progressLessons]
  );
  ...
  const filteredLessons = useMemo(() => {
    ...
    return allLessons.filter((lesson) => {
      const matchesQuery = !normalized || [lesson.title, lesson.description ?? '', lesson.categoryName]
        .join(' ')
        .toLowerCase()
        .includes(normalized);
  ...
  const filteredIds = useMemo(() => new Set(filteredLessons.map((lesson) => lesson.id)), [filteredLessons]);
    ...
    <div className="max-w-6xl mx-auto space-y-6 pb-10">
      <section className="relative overflow-hidden rounded-3xl bg-gradient-to-l from-indigo-700 via-indigo-600 to-blue-500 p-6 sm:p-8 text-white shadow-xl shadow-indigo-100">
        ...
        <div className="relative flex flex-col lg:flex-row lg:items-end lg:justify-between gap-6">
          <div>
            <div className="inline-flex items-center gap-2 rounded-full bg-white/10 px-3 py-1.5 text-xs font-bold text-indigo-50 mb-4">
            ...
            <p className="text-indigo-100 max-w-2xl leading-relaxed">
              שיעורים מסודרים, תרגול ממוקד והתקדמות אישית — הכול במקום אחד.
            </p>
          </div>
          <Link
            href="/practice"
            className="inline-flex items-center justify-center gap-2 rounded-xl bg-white px-5 py-3 text-sm font-bold text-indigo-700 shadow-lg hover:bg-indigo-50 transition-colors shrink-0"
          >
        ...
        <section className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
          <Card className="p-4 sm:p-5">
            <div className="flex items-center gap-3">
              ...
              <div><div className="text-xl font-extrabold text-slate-900">{totalLessons}</div><div className="text-xs text-slate-500">שיעורים</div></div>
            </div>
          </Card>
          <Card className="p-4 sm:p-5">
            <div className="flex items-center gap-3">
              ...
              <div><div className="text-xl font-extrabold text-slate-900">{completedCount}</div><div className="text-xs text-slate-500">הושלמו</div></div>
            </div>
          </Card>
          <Card className="p-4 sm:p-5">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-xl bg-amber-50 text-amber-600 flex items-center justify-center"><Trophy className="w-5 h-5" /></div>
              <div><div className="text-xl font-extrabold text-slate-900">{dashboardQuery.data?.xp_total ?? 0}</div><div className="text-xs text-slate-500">XP שנצברו</div></div>
            </div>
          </Card>
          <Card className="p-4 sm:p-5">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-xl bg-rose-50 text-rose-600 flex items-center justify-center"><Flame className="w-5 h-5" /></div>
              <div><div className="text-xl font-extrabold text-slate-900">{accuracy}%</div><div className="text-xs text-slate-500">דיוק בתרגול</div></div>
            </div>
          </Card>
        </section>
      )}
      ...
      {continueLesson && (
        <Link
          href={`/lesson/${continueLesson.lesson_id}`}
          className="group flex items-center gap-4 rounded-2xl border border-indigo-100 bg-indigo-50/70 p-4 sm:p-5 hover:bg-indigo-50 transition-colors"
        >
          ...
          <div className="flex-1 min-w-0">
            <div className="text-xs font-bold text-indigo-600 mb-0.5">המשך מהנקודה האחרונה</div>
            <div className="font-bold text-slate-900 truncate">{continueLesson.title}</div>
            <div className="text-xs text-slate-500 truncate">{continueLesson.category_name}</div>
          </div>
          <ArrowLeft className="w-5 h-5 text-indigo-500 group-hover:-translate-x-1 transition-transform" />
        </Link>
      )}
      ...
      {categories.length > 0 && (
        <>
          ...
          {filteredLessons.length > 0 && (
            <section>
              <div className="flex items-center justify-between mb-4">
                <div>
                  <h2 className="text-xl font-extrabold text-slate-900">השיעורים</h2>
                  <p className="text-sm text-slate-500 mt-1">בחר שיעור והתקדם בקצב שלך</p>
                </div>
              </div>
          ...
          {filteredLessons.length === 0 && (
            <EmptyState
              icon={<BookOpen className="w-7 h-7" />}
              title="לא מצאנו שיעורים מתאימים"
              description="נסו לשנות את החיפוש או את רמת הקושי."
              action={
                <button
                  type="button"
          ...
          {!query && difficulty === 'all' && (
            <section className="pt-2">
              <div className="flex items-end justify-between mb-4">
                <div>
                  <h2 className="text-xl font-extrabold text-slate-900">תחומי הלמידה</h2>
                  <p className="text-sm text-slate-500 mt-1">העמק לפי התחום שמעניין אותך</p>
                </div>
              </div>
