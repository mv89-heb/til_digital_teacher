...
type Intent = 'greeting' | 'explain' | 'practice' | 'mistake' | 'question' | 'topic' | 'strategy' | 'difficulty' | 'fallback';
...
function classifyIntent(input: string): Intent {
  ...
  if (/(לא מבין|לא הבנתי|תסביר|הסבר|מה זה|מה הכוונה)/.test(text)) return 'explain';
  ...
  return questions
    .map((question) => {
      const haystack = normalize([
        questionText(question), question.main_category, question.subcategory, question.skill,
        question.question_type, ...(question.tags ?? []),
      ].filter(Boolean).join(' '));
...
const topicLabel = (question?: Question | null) => question?.subcategory || question?.skill || question?.main_category || question?.question_type || 'הנושא הנוכחי';

function buildLocalTeacherReply(input: string, mode: TeacherMode, categories: Category[], questions: Question[], selectedQuestion?: Question | null): string {
  const intent = classifyIntent(input);
  const relevant = selectedQuestion ? [selectedQuestion] : findRelevantQuestions(input, questions);
  const current = relevant[0];
  const availableTopics = categories.flatMap((category) => category.lessons.map((lesson) => lesson.title)).slice(0, 10);

  if (intent === 'greeting') return 'שלום! אני המורה המקומי של המערכת. אני מכיר את קטלוג הלימוד ואת השאלות שנטענו למערכת, ויכול ללמד נושא, לפרק שאלה, לתת רמזים, לתרגל ולנתח טעויות.\n\nאפשר להתחיל ב״למד אותי נושא״ או לשלוח שאלה ספציפית.';

  if (intent === 'practice' || mode === 'practice') {
    if (current) return `מצוין. נתרגל עכשיו את ${topicLabel(current)}.\n\nאני לא אגלה את התשובה מיד. קודם נסה לזהות מה השאלה בודקת ומה הצעד הראשון שהיית עושה. אם תרצה, כתוב את האפשרות שבחרת ואבדוק אותה איתך.`;
    return 'מעולה. בחר נושא או רמת קושי באזור התרגול, ואני אבנה לך תרגול מתוך מאגר השאלות הקיים. המטרה היא להבין את שיטת הפתרון ולא רק לצבור תשובות נכונות.';
  }

  if (intent === 'mistake' || mode === 'mistake') {
    if (current) {
      const solution = cleanRichText(current.solution_text);
      return `בוא ננתח את הטעות בשאלה הזו.\n\nהשאלה שייכת ל: ${topicLabel(current)}.\n\nהצעד הראשון הוא לא לשאול רק "מה התשובה?", אלא "איזה כלל או קשר השאלה דורשת לזהות?".${solution ? `\n\nפתרון המערכת: ${solution}` : '\n\nלשאלה הזו אין כרגע פתרון טקסטואלי מלא במידע שהתקבל מהמערכת.'}\n\nאם תכתוב לי איזו אפשרות בחרת, אוכל להסביר מה גרם לטעות ומה כדאי לבדוק בפעם הבאה.`;
    }
  ...
  if (intent === 'strategy') {
    if (current) return `בשאלה הזו כדאי לעבוד לפי סדר קבוע:\n\n1. לזהות מה בדיוק מבקשים.\n2. לזהות את סוג השאלה: ${current.question_type || 'סוג השאלה'}.\n3. להגדיר את הקשר או הכלל המרכזי לפני שמסתכלים על המסיחים.\n4. לפסול אפשרויות שאינן עומדות בכלל.\n5. לבדוק את התשובה פעם נוספת מול נוסח השאלה.\n\nהמיומנות המרכזית כאן היא: ${topicLabel(current)}.`;
    return 'שיטת העבודה של המורה היא: קודם להבין מה השאלה בודקת, אחר כך לזהות את סוג המשימה, לבנות כלל או דרך פתרון, ורק בסוף להשתמש באפשרויות התשובה כדי לאמת את המסקנה. כך נמנעים מניחוש בין מסיחים.';
  }

  if (intent === 'question' && current) {
    const answers = current.answers.slice().sort((a, b) => a.order - b.order).map((answer, index) => `${index + 1}. ${answer.answer_text}`).join('\n');
    return `מצאתי שאלה רלוונטית במאגר.\n\n${questionText(current)}\n\nאפשרויות:\n${answers || 'אין אפשרויות זמינות'}\n\nאני מציע לא לבחור מיד. נסה להסביר לי מה הקשר או הכלל שאתה רואה, ואז נבדוק יחד.`;
  }

  if (intent === 'difficulty') return 'אפשר ללמוד בהדרגה: קל → בינוני → בחינה. אני ממליץ להתחיל ברמה שבה אתה מצליח להסביר את דרך הפתרון, לא רק לסמן תשובה נכונה, ואז לעלות רמה.';

  if (intent === 'topic' || intent === 'explain') {
    const inputTokens = normalize(input).split(' ').filter((token) => token.length > 2);
    const lessonMatch = availableTopics.find((topic) => inputTokens.some((token) => normalize(topic).includes(token)));
    if (lessonMatch) return `בשמחה. נתחיל מ־${lessonMatch}.\n\nהמטרה שלי היא ללמד בשלושה שלבים: להבין את הרעיון, לראות דוגמה, ואז לפתור שאלה בעצמך.\n\nבשלב הראשון אסביר את העיקרון בשפה פשוטה. בשלב השני נפרק שאלה לדוגמה. בשלב השלישי אתן לך לנסות, ואספק רמזים לפני שאגלה את הפתרון.`;
    if (current) return `בוא נלמד דרך השאלה שמצאתי. היא עוסקת ב־${topicLabel(current)}.\n\nאל תתחיל מהתשובות. קודם נסח במילים שלך מה מבקשים ממך. אחר כך נזהה את הכלל המתאים ונבדוק כל אפשרות מולו.\n\nאם תרצה, כתוב "תן רמז" ואעבור איתך שלב אחד בכל פעם.`;
    return `בשמחה. המורה מחובר לקטלוג הלימוד המקומי ולכן הוא יכול ללמד מתוך התוכן שקיים במערכת, בלי API חיצוני.\n\nהנושאים הזמינים כרגע כוללים: ${availableTopics.join(', ') || 'הקטלוג נטען כעת'}.\n\nכתוב את שם הנושא ואסביר אותו מהבסיס ועד תרגול.`;
  }

  if (current) return `אני מזהה שאלה רלוונטית בנושא ${topicLabel(current)}.\n\nבוא נפתור אותה כמו בכיתה: קודם נבין מה מבקשים, אחר כך נבחר אסטרטגיה, ורק אז נבדוק את התשובות.\n\nאם אתה רוצה הסבר מלא, כתוב "תסביר". אם אתה רוצה ללמוד לבד, כתוב "תן רמז".`;
...
async function loadAllQuestions(token: string, search?: string): Promise<Question[]> {
  const first = await getQuestionBank(token, { page: 1, perPage: 100, search: search || undefined });
  const all = [...first.questions];
  for (let page = 2; page <= first.total_pages; page += 1) {
    const next = await getQuestionBank(token, { page, perPage: 100, search: search || undefined });
...
export default function AITeacherPage() {
  ...
  const [messages, setMessages] = useState<Message[]>([
    { role: 'assistant', text: 'שלום! אני המורה האישי שלך. אני עובד ללא API חיצוני ומבוסס על חומרי הלימוד והשאלות שקיימים במערכת. אפשר לבקש ממני ללמד נושא, להסביר שאלה, לתת רמז, לתרגל או לנתח טעות.' },
  ]);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        if (token) {
          const allQuestions = await loadAllQuestions(token);
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      try {
        const filtered = await loadAllQuestions(token, search.trim());
        setQuestions(filtered);
      } catch {
        // Keep the last successful knowledge set when a search request fails.
      }
    }, 350);
  ...
  const stats = useMemo(() => ({
    ...
    lessons: categories.reduce((sum, category) => sum + category.lesson_count, 0),
  ...
  const send = async (event: FormEvent) => {
    ...
    setInput('');
    try {
      const reply = buildLocalTeacherReply(prompt, mode, categories, questions, selectedQuestion);
        ...
        <div className="flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
          <div>
            <div className="mb-3 flex items-center gap-3">
              ...
              <div><p className="text-sm font-medium text-indigo-100">מרכז ההוראה החכם</p><h1 className="text-3xl font-black md:text-4xl">המורה האישי שלך</h1></div>
            </div>
            <p className="max-w-2xl text-indigo-50">מורה מקומי שמכיר את תוכן הלימוד ואת מאגר השאלות של המערכת. הוא מסביר, שואל שאלות נגדיות, נותן רמזים ומלמד שיטת פתרון — בלי API חיצוני.</p>
          </div>
        ...
        <aside className="space-y-4">
          <Card className="p-4">
            ...
            <div className="space-y-2">
              {([
                ['learn', 'למד אותי', 'הסבר מסודר מהבסיס'], ['guided', 'פתרון מודרך', 'רמזים לפני תשובה'], ['practice', 'תרגול', 'שאלה אחת בכל פעם'], ['mistake', 'ניתוח טעות', 'להבין למה טעינו'],
              ] as const).map(([value, title, subtitle]) => (
                <button key={value} type="button" onClick={() => setMode(value)} className={`w-full rounded-xl border p-3 text-right transition ${mode === value ? 'border-indigo-300 bg-indigo-50 text-indigo-900' : 'border-slate-200 bg-white hover:bg-slate-50'}`}>
                  <div className="font-semibold">{title}</div><div className="mt-0.5 text-xs text-slate-500">{subtitle}</div>
                </button>
              ))}
            </div>
          </Card>
          ...
          <Card className="p-4">
            ...
            <input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="חפש שאלה או נושא..." className="w-full rounded-xl border border-slate-300 px-3 py-2.5 text-sm outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-100" />
            <div className="mt-3 max-h-72 space-y-2 overflow-y-auto">
              {questions.slice(0, 12).map((question) => (
                <button key={question.id} type="button" onClick={() => { setSelectedQuestion(question); setMode('guided'); ask('תסביר לי את השאלה שבחרתי שלב־אחר־שלב.'); }} className={`w-full rounded-xl border p-3 text-right text-sm hover:border-indigo-300 hover:bg-indigo-50 ${selectedQuestion?.id === question.id ? 'border-indigo-300 bg-indigo-50' : 'border-slate-200'}`}>
        ...
        <main className="min-w-0 space-y-4">
          <Card className="overflow-hidden">
            ...
            <div className="min-h-[500px] max-h-[620px] space-y-4 overflow-y-auto bg-slate-50 p-4 md:p-6">
              {messages.map((message, index) => <div key={`${index}-${message.role}`} className={`flex ${message.role === 'user' ? 'justify-start' : 'justify-end'}`}><div className={`max-w-[88%] rounded-2xl px-4 py-3 shadow-sm md:max-w-[78%] ${message.role === 'user' ? 'bg-indigo-600 text-white' : 'border border-slate-200 bg-white text-slate-800'}`}><div className="mb-1 flex items-center gap-2 text-xs font-bold opacity-70">{message.role === 'user' ? 'אתה' : <><Brain className="h-3.5 w-3.5" /> המורה</>}</div><div className="whitespace-pre-wrap leading-7">{message.text}</div></div></div>)}
            ...
            <form onSubmit={send} className="border-t border-slate-200 bg-white p-4">
              <div className="flex gap-3">
                <input id="ai-teacher-input" value={input} onChange={(event) => setInput(event.target.value)} placeholder="שאל את המורה כל דבר על הלמידה..." className="min-w-0 flex-1 rounded-2xl border border-slate-300 px-4 py-3 outline-none transition focus:border-indigo-500 focus:ring-2 focus:ring-indigo-100" />
                <button type="submit" disabled={sending || !input.trim()} className="rounded-2xl bg-indigo-600 px-5 text-white transition hover:bg-indigo-700 disabled:cursor-not-allowed disabled:opacity-50" aria-label="שלח"><Send className="h-5 w-5" /></button>
              </div>
              <p className="mt-2 text-xs text-slate-500">הסברים מבוססים על תוכן הלימוד והמידע שנטען מהמערכת. אין חיבור לספק AI חיצוני.</p>
            </form>
          </Card>

          {selectedQuestion && <Card className="border-indigo-100 bg-indigo-50/50 p-5"><div className="mb-3 flex items-center gap-2 font-bold text-indigo-900"><Trophy className="h-5 w-5" /> השאלה שנבחרה</div><p className="leading-7 text-slate-800">{questionText(selectedQuestion)}</p><div className="mt-4 flex flex-wrap gap-2 text-xs"><span className="rounded-full bg-white px-3 py-1.5 text-slate-600">{topicLabel(selectedQuestion)}</span><span className="rounded-full bg-white px-3 py-1.5 text-slate-600">{selectedQuestion.difficulty}</span>{selectedQuestion.skill && <span className="rounded-full bg-white px-3 py-1.5 text-slate-600">מיומנות: {selectedQuestion.skill}</span>}</div><div className="mt-4 flex flex-wrap gap-2"><button type="button" onClick={() => { setMode('guided'); ask('תן לי רמז בלבד לשאלה הזו, בלי לגלות את התשובה.'); }} className="inline-flex items-center gap-2 rounded-xl bg-white px-3 py-2 text-sm font-semibold text-indigo-700 shadow-sm"><Lightbulb className="h-4 w-4" /> תן רמז</button><button type="button" onClick={() => { setMode('learn'); ask('הסבר לי את הפתרון המלא לשאלה הזו ולמה כל שלב נכון.'); }} className="inline-flex items-center gap-2 rounded-xl bg-white px-3 py-2 text-sm font-semibold text-indigo-700 shadow-sm"><CheckCircle2 className="h-4 w-4" /> הסבר פתרון</button></div></Card>}
          ...
          {loading && <div className="flex items-center justify-center gap-2 py-2 text-sm text-slate-500"><MessageCircle className="h-4 w-4 animate-pulse" /> טוען את כל הידע המקומי של המורה...</div>}
        </main>
      </div>
    </div>
  );
