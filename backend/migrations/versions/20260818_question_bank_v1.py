...
def _get_or_create_category(conn, name, category_type, parent_id=None, description=None, order=0):
    row = conn.execute(
        sa.text(
            ...
            "AND ((parent_id=:parent_id) OR (parent_id IS NULL AND :parent_id IS NULL)) LIMIT 1"
        ),
    ...
    return conn.execute(
        sa.text(
            ...
            "VALUES (:name, :description, :type, 'published', :parent_id, :order, NOW(), NOW()) RETURNING id"
        ),
        {"name": name, "description": description, "type": category_type, "parent_id": parent_id, "order": order},
    ).scalar_one()


def _insert_question(conn, category_id, payload):
    existing = conn.execute(
        sa.text("SELECT id FROM questions WHERE question_metadata ->> 'bank_key' = :bank_key LIMIT 1"),
    ...
    metadata = {
        ...
        "quality": {"review_status": "APPROVED", "single_correct_answer": True, "source": "original_til_bank_v1"},
    }

    question_id = conn.execute(
        sa.text(
            "INSERT INTO questions "
            "(category_id, question_type, difficulty, status, body, solution_text, recommended_time_seconds, question_metadata, created_at, updated_at) "
            "VALUES (:category_id, 'multiple_choice', :difficulty, 'published', CAST(:body AS JSON), CAST(:solution AS JSON), :time_seconds, CAST(:metadata AS JSON), NOW(), NOW()) RETURNING id"
        ),
        {
    ...
    for position, answer in enumerate(payload["answers"], start=1):
        answer_id = conn.execute(
            sa.text(
                ...
                "VALUES (:question_id, :answer_text, :is_correct, CAST(:explanation AS JSON), :position, NOW(), NOW()) RETURNING id"
            ),
            {
    ...
    conn.execute(
        sa.text(
            ...
            "(question_id, version_number, category_id, question_type, difficulty, status, body, solution_text, question_metadata, answer_snapshot, recommended_time_seconds, created_at) "
            "SELECT :question_id, 1, category_id, question_type, difficulty, status, body, solution_text, question_metadata, CAST(:snapshot AS JSONB), recommended_time_seconds, NOW() "
            "FROM questions WHERE id=:question_id"
        ),
        {"question_id": question_id, "snapshot": json.dumps(snapshot, ensure_ascii=False)},
    )
    return question_id
...
def upgrade():
    ...
    questions = [
        ("quant_percent", {
            ...
            "solution": "נניח שהמחיר המקורי הוא 100. לאחר העלייה 125, ולאחר הירידה 100. לכן המחיר הסופי שווה למחיר המקורי.",
            "answers": [
                {"text": "95%", "is_correct": False, "explanation": "הירידה מחושבת מתוך 125."},
        ...
        ("quant_rate", {
            ...
            "solution": "בשעתיים הראשונות יוצרו 400 יחידות. בשלוש השעות הבאות מכונה ב' יצרה 240. סך הכול 640.",
            "answers": [
                {"text": "480", "is_correct": False, "explanation": "מתעלם מהעבודה של ב' לאחר שא' הפסיקה."},
        ...
        ("quant_sequence", {
            ...
            "solution": "ההפרשים הם 4, 6, 8, 10. ההפרש הבא הוא 12, ולכן התשובה 42.",
            "answers": [
        ...
        ("verbal_analogy", {
            ...
            "solution": "רופא מעניק טיפול למטופל; באופן דומה מורה מעניק הוראה לתלמיד.",
            "answers": [
                {"text": "מורה : תלמיד", "is_correct": True, "explanation": "אותו יחס של נותן טיפול או הכוונה למקבל."},
                {"text": "ספר : ספרייה", "is_correct": False, "explanation": "פריט ומקום."},
                {"text": "מכונית : כביש", "is_correct": False, "explanation": "כלי וסביבה."},
                {"text": "סכין : מטבח", "is_correct": False, "explanation": "כלי ומקום שימוש."},
        ...
        ("verbal_sentence", {
            ...
            "solution": "הפתיח יוצר ניגוד לציפייה: הנתונים הפריכו את ההשערה ולכן החוקר נדרש לעדכן את המודל.",
            "answers": [
                {"text": "חיזקו / לדחות", "is_correct": False, "explanation": "אינו מתאים למבנה הניגודי."},
                {"text": "הפריכו / לעדכן", "is_correct": True, "explanation": "נכון."},
                {"text": "אישרו / להרחיב", "is_correct": False, "explanation": "אין כאן ניגוד לציפייה."},
                {"text": "הסבירו / להוכיח", "is_correct": False, "explanation": "אינו מתאים ליחסים במשפט."},
        ...
        ("fig_rotation", {
            ...
            "solution": "בסיבוב של 90° בכיוון השעון הזרוע האנכית הופכת לאופקית והזרוע האופקית פונה כלפי מטה. אין לבצע שיקוף.",
        ...
        ("fig_matrix", {
            ...
            "solution": "בשורה השלישית החוק הוא 3, 5, 7. לכן התא החסר מכיל 7 נקודות.",
            "visual": {"format": "matrix", "cells": [[1,3,5],[2,4,6],[3,5,None]], "rule": "+2 across each row"},
            "answers": [
        ...
        ("fig_cube", {
            ...
            "solution": "מהנתונים ידועות שלוש פאות סמוכות בלבד. בלי לדעת את ציר הסיבוב והפאות הנגדיות, אי אפשר לקבוע בוודאות את הפאה הקדמית.",
            ...
            "answers": [
                ...
                {"text": "משולש", "is_correct": False, "explanation": "ייתכן אך אינו הכרחי."},
                {"text": "ריבוע", "is_correct": False, "explanation": "הריבוע עבר למעלה."},
                {"text": "לא ניתן לקבוע", "is_correct": True, "explanation": "נכון: חסר מידע על הכיוון והפאות הנגדיות."},
